...
extern void *_binary_shellcode_o_start[];
extern void *_binary_shellcode_o_end[];
extern size_t _binary_shellcode_o_size;
...
void generateShellcode(char *orig_filename, u64 orig_size) {
	...
	// Shellcode first
	size_t shellcode_size = (size_t)_binary_shellcode_o_end - (size_t)_binary_shellcode_o_start;
	printf("\t -> Writing Shellcode (%lu bytes)... ", shellcode_size);
	printf(" -> Data %p -> %p ", _binary_shellcode_o_start, _binary_shellcode_o_end);
	i = fwrite (_binary_shellcode_o_start, 1, shellcode_size - 1, inject_file);
