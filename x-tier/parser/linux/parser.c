...
extern size_t _binary_shellcode_bin_size;
...
void generateShellcode(const char *input_filename, const char *out_filename, u64 orig_size) {
	...
	// Shellcode first
	size_t shellcode_size = (size_t)_binary_shellcode_bin_end - (size_t)_binary_shellcode_bin_start;
	printf("\t -> Writing Shellcode (%lu bytes)... ", shellcode_size);

	printf(" -> Data %p -> %p ", _binary_shellcode_bin_start, _binary_shellcode_bin_end);
	i = fwrite (_binary_shellcode_bin_start, 1, shellcode_size - 1, inject_file);
...
int main(int argc, char *argv[])
{
	...
	// Go for it
	generateShellcode(input_file_name, output_file_name, elf_stats.st_size);
